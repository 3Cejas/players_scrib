...
test("the finished-writing scene fully replaces the old spectator HUD", () => {
  ...
  assert.match(sockets, /function teleprompterActivoEspectador\(\)[\s\S]*teleprompter\.classList\.contains\("activo"\)[\s\S]*preparando\.classList\.contains\("activo"\)/);
  ...
  assert.match(read("game/spectator/js/state.js"), /overlay\.classList\.toggle\("activo", teleprompter_estado\.visible\);[\s\S]*teleprompter_preparing\.classList\.toggle\("activo", preparando\);[\s\S]*sincronizarCierrePartidaEspectadorConVista\(vista_espectador_modo_resuelta\)/);
});
