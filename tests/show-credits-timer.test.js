...
test("credits are rendered and scored locally on spectator and muse screens", () => {
  ...
  assert.match(museCss, /\.creditos-musa__track\s*\{[\s\S]*contain:\s*layout paint style;/);
  assert.match(museCss, /\.creditos-musa:not\(\.creditos-musa--finalizados\)[\s\S]*animation-play-state:\s*paused;/);
...
test("giant timer is a dedicated synced scene on spectator and muse screens", () => {
  ...
  assert.match(spectatorState, /function temporizadorGiganteActivoEspectador\(\)/);
  assert.match(spectatorState, /iniciarTemporizadorGigante[\s\S]*ocultarCierrePartidaEspectador/);
  ...
  assert.match(spectatorCss, /\.temporizador-gigante\s*\{[\s\S]{0,700}z-index:\s*2147483000;/);
...
test("credits editor and result use the standard section button style", () => {
  ...
  assert.match(controlHtml, /id="boton_banderas_musas_final" class="[^"]*btn-estandar-seccion[^"]*btn-banderas-musas-control[^"]*"/);
  assert.equal((controlHtml.match(/data-banderas-musas-control[^>]*>\s*&#x1F6A9; BANDERA</g) || []).length, 2);
  assert.doesNotMatch(controlHtml, /BANDERAS (?:DES)?ACTIVADAS/);
  ...
  assert.match(controlCss, /\.btn-banderas-musas-control\s*\{[\s\S]*\.btn-banderas-musas-control\[data-active="1"\]/);
