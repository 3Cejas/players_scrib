...
test("muse onboarding separates the three animated rules and keeps the original welcome", () => {
  ...
  assert.match(html, /onboarding-disadvantage-meter[\s\S]*TU EQUIPO[\s\S]*EQUIPO RIVAL/);
  ...
  assert.match(html, /#intro-nombre \.intro-logos img \{[\s\S]{0,120}animation: none;/);
  assert.doesNotMatch(html, /@keyframes onboardingNameLogo/);
