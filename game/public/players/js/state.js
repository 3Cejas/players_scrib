...
function mostrarRegaloPdf(payload) {
    ...
    if (!puedeMostrarRegaloPdfMusa() && payload.debug !== true) {
...
function traducirPostgameMusa(clave, fallback, variables = {}) {
    return window && typeof window.scribT2P === "function" ? window.scribT2P(clave, variables, fallback) : fallback;
}
...
function colorPostgameEscritxr(playerId) {
    return Number(playerId) === 2 ? "#ff6578" : "#43eaff";
}

function limitarPostgame(valor, min = 0, max = 100) {
    return Math.max(min, Math.min(max, Number(valor) || 0));
}

function fijarBarraPostgameMusa(id, porcentaje) {
    const barra = getEl(id);
    if (barra) barra.style.setProperty("--postgame-bar-pct", `${limitarPostgame(porcentaje)}%`);
}

function pdfEscritxrPostgameMusa(playerId) {
    const id = Number(playerId) === 2 ? 2 : 1;
    const escritxr = escritorPostgameMusa(id);
    const pdf = escritxr && escritxr.pdf && typeof escritxr.pdf === "object" ? escritxr.pdf : null;
    if (pdf && pdf.data) return pdf;
    const equipo = Number(regalo_postgame_data && regalo_postgame_data.player) === 2 ? 2 : 1;
    if (id === equipo && regalo_pdf_ultimo_data) {
        return { data: regalo_pdf_ultimo_data, filename: regalo_pdf_ultimo_filename };
    }
    return null;
}
...
function pintarComparativaEscritoresPostgameMusa() {
    ...
    escritores.forEach(({ id, datos }) => {
        ...
        if (botonPdf) {
            ...
            botonPdf.setAttribute("aria-label", `${traducirPostgameMusa("muse.postgame.download", "Descargar")} ${nombre}`);
        }
    });
}
...
function pintarRankingPostgameMusa() {
    ...
    lista.classList.toggle("is-scrollable", ranking.length > 5);
}
...
function pintarPostgameMusa() {
    ...
    const enviadas = Math.max(0, Number(stats.enviadas) || 0);
    const introducidas = Math.max(0, Number(stats.introducidas) || 0);
    ...
    fijarBarraPostgameMusa("musa_postgame_bar_letras", ((Number(stats.letras) || 0) / totalTipos) * 100);
    if (musa_postgame_tab_propio) {
        musa_postgame_tab_propio.dataset.player = String(equipo);
        musa_postgame_tab_propio.textContent = escritorPostgameMusa(equipo).nombre || `ESCRITXR ${equipo}`;
        musa_postgame_tab_propio.style.setProperty("--postgame-tab-color", colorPostgameEscritxr(equipo));
    }
    if (musa_postgame_tab_rival) {
        musa_postgame_tab_rival.dataset.player = String(rival);
        musa_postgame_tab_rival.textContent = escritorPostgameMusa(rival).nombre || `ESCRITXR ${rival}`;
        musa_postgame_tab_rival.style.setProperty("--postgame-tab-color", colorPostgameEscritxr(rival));
    }
...
async function descargarPdfEscritxrPostgameMusa(playerId, boton) {
    ...
    boton.disabled = true;
    try {
        await descargarArchivoRegalo(pdf.data, pdf.filename);
    } catch (error) {
...
if (musa_postgame_pdf_j1) {
    musa_postgame_pdf_j1.addEventListener("click", async () => {
        await descargarPdfEscritxrPostgameMusa(1, musa_postgame_pdf_j1);
    });
}
if (musa_postgame_pdf_j2) {
    musa_postgame_pdf_j2.addEventListener("click", async () => {
        await descargarPdfEscritxrPostgameMusa(2, musa_postgame_pdf_j2);
    });
}
...
const resultado_final_musa = getEl("resultado_final_musa");
const resultado_final_musa_stage = getEl("resultado_final_musa_stage");
let estado_puntuacion_final_musa = null;
let estado_resultado_jurado_musa = null;
let estado_resultado_final_musa = null;
let puntuacion_slide_step_musa = 0;
let puntuacion_reveal_phase_musa = 0;
let jurado_slide_step_musa = 0;
let puntuacion_firma_render_musa = "";
let jurado_firma_render_musa = "";
...
function marcadorPuntuacionJuradoMusa(valorEntrada) {
    const valor = Math.max(0, Math.min(10, Number(valorEntrada) || 0));
    const porcentaje = valor * 10;
    const matiz = Math.round(porcentaje * 1.2);
    return `<div class="resultado-musa__jury-score" style="--jury-score:${porcentaje.toFixed(1)}%;--jury-score-hue:${matiz}"><div class="resultado-musa__jury-score-track" aria-hidden="true"><i></i><span></span></div><b>${valor.toFixed(1)}<em>/10</em></b></div>`;
}
...
function tarjetaJuradoDirectoMusa(estado, id, criterio) {
    ...
    return `<article class="resultado-musa__card resultado-musa__card--jurado-live resultado-musa__card--${id}">
        <small>${confirmado ? "PUNTUACI&Oacute;N CONFIRMADA" : "EN DIRECTO"}</small>
        <h3>${escapeHtml(jugador.nombre)}</h3>
        ${marcadorPuntuacionJuradoMusa(valor)}
    </article>`;
}

function tarjetaResumenJuradoMusa(jugador, id) {
    const total = Math.max(0, Math.min(10, Number(jugador && jugador.total) || 0));
    return `<article class="resultado-musa__card resultado-musa__card--jurado-live resultado-musa__card--${id}">
        <small>PUNTUACI&Oacute;N MEDIA</small>
        <h3>${escapeHtml(jugador && jugador.nombre ? jugador.nombre : `ESCRITXR ${id}`)}</h3>
        ${marcadorPuntuacionJuradoMusa(total)}
    </article>`;
}
...
function aplicarGanadorLocalMusa(pantalla, ganador, firma) {
    if (!pantalla) return;
...
function tarjetaApartadoMusa(estado, id, valores, ganador, escala = 10, puntos = null, unidad = "") {
    ...
    return `<article class="resultado-musa__card resultado-musa__card--${id}${gana ? " is-winner" : ""}">
        <small>${gana ? "GANA" : "FINALISTA"}</small>
        <h3>${escapeHtml(jugador.nombre)}</h3>
        <strong>${valor.toFixed(1)}</strong><span>${unidad ? escapeHtml(String(unidad).toUpperCase()) : `/ ${escala}`}</span>
        ${puntos ? `<b>+${Number(puntos[id] || 0).toFixed(1)} PTS</b>` : ""}
    </article>`;
}

function proporcionTotalVideojuegoMusa(totales = {}) {
    const suma = (Number(totales[1]) || 0) + (Number(totales[2]) || 0);
    return suma > 0 ? Math.max(8, Math.min(92, ((Number(totales[1]) || 0) / suma) * 100)) : 50;
}

function marcadorTotalVideojuegoMusa(estado, categoriasReveladas, totalesForzados = null, totalesObjetivo = null) {
    const api = window.ScribFinalScore;
...
function tarjetaApartadoVideojuegoMusa(estado, categoria, id, fase, mostrarGanador = true) {
    ...
    const altura = peso > 0 ? Math.max(0, Math.min(100, (puntos / peso) * 100)) : 0;
    const recienRevelado = fase === id;
    if (!revelado) {
        return `<article class="resultado-musa__card resultado-musa__card--comparativa resultado-musa__card--${id} is-concealed">
            <h3>${escapeHtml(jugador.nombre)}</h3>
            <div class="resultado-musa__misterio" aria-label="Puntuaci&oacute;n por desvelar">?</div>
        </article>`;
    }
    return `<article class="resultado-musa__card resultado-musa__card--comparativa resultado-musa__card--${id}${gana ? " is-winner" : ""}${revelado ? " is-revealed" : " is-concealed"}${recienRevelado ? " is-newly-revealed" : ""}">
        <small>${revelado ? "DESVELADO" : "EN MISTERIO"}</small>
        <h3>${escapeHtml(jugador.nombre)}</h3>
        <div class="resultado-musa__dato"><strong data-score-final="${valor}">${valor.toFixed(1)}</strong><span>${escapeHtml(String(categoria.unidad || "").toUpperCase())}</span></div>
        <div class="resultado-musa__barra-vertical" data-points-source="${id}"><i style="--resultado-fill:${altura.toFixed(2)}%"></i><b><span data-points-final="${puntos}">+${puntos.toFixed(1)}</span><small>PTS</small></b></div>
    </article>`;
}

function cancelarTransferenciaPuntuacionMusa() {
    if (puntuacion_timeout_transferencia_musa) {
        clearTimeout(puntuacion_timeout_transferencia_musa);
        puntuacion_timeout_transferencia_musa = null;
    }
    puntuacion_raf_totales_musa.forEach((id) => cancelAnimationFrame(id));
    puntuacion_raf_totales_musa = [];
    resultado_videojuego_musa?.querySelectorAll(".resultado-musa__puntos-vuelo").forEach((elemento) => elemento.remove());
}
...
function animarCifraPuntuacionMusa(elemento, final, prefijo = "") {
    ...
    const tick = (ahora) => {
        ...
        const progreso = Math.min(1, (ahora - inicio) / 1250);
        if (progreso < 1) {
            elemento.textContent = `${prefijo}${(Math.random() * Math.max(4, numeroFinal * 1.55)).toFixed(1)}`;
            const raf = requestAnimationFrame(tick);
            puntuacion_raf_totales_musa.push(raf);
            return;
        }
        elemento.textContent = `${prefijo}${numeroFinal.toFixed(1)}`;
...
function iniciarRuletaCategoriaPuntuacionMusa() {
    const tarjeta = resultado_videojuego_musa_stage?.querySelector(".resultado-musa__card.is-newly-revealed");
    if (!tarjeta) return;
    const metrica = tarjeta.querySelector("[data-score-final]");
    const puntos = tarjeta.querySelector("[data-points-final]");
    animarCifraPuntuacionMusa(metrica, metrica?.dataset.scoreFinal);
    animarCifraPuntuacionMusa(puntos, puntos?.dataset.pointsFinal, "+");
}

function actualizarMarcadorTotalPuntuacionMusa(totalesObjetivo, animar = true) {
    const barra = resultado_videojuego_musa_stage?.querySelector(".resultado-musa__total-barra");
    if (!barra) return;
    const inicio = { 1: Number(barra.dataset.total1) || 0, 2: Number(barra.dataset.total2) || 0 };
    const objetivo = { 1: Number(totalesObjetivo?.[1]) || 0, 2: Number(totalesObjetivo?.[2]) || 0 };
    ...
    [1, 2].forEach((id) => {
        ...
        const tick = (ahora) => {
            ...
            const valor = inicio[id] + ((objetivo[id] - inicio[id]) * (1 - Math.pow(1 - t, 3)));
...
function transferirPuntosAlMarcadorMusa(id, puntos, totalesObjetivo) {
    const origen = resultado_videojuego_musa_stage?.querySelector(`[data-points-source="${id}"] b`);
    const destino = resultado_videojuego_musa_stage?.querySelector(`.resultado-musa__total-barra i:nth-child(${id})`);
    if (!origen || !destino || !resultado_videojuego_musa) {
    ...
    puntuacion_timeout_transferencia_musa = setTimeout(() => {
        ...
        resultado_videojuego_musa.appendChild(vuelo);
        requestAnimationFrame(() => vuelo.classList.add("is-flying"));
        vuelo.addEventListener("animationend", () => {
            vuelo.remove();
            actualizarMarcadorTotalPuntuacionMusa(totalesObjetivo, true);
        }, { once: true });
    }, 1420);
}

function renderizarPuntuacionFinalMusa(opciones = {}) {
    ...
    const vista = api ? api.obtenerVista(estado, puntuacion_slide_step_musa) : { tipo: "final", paso: 0, estado };
    const fase = vista.tipo === "categoria" && api && typeof api.normalizarFaseRevelado === "function"
        ? api.normalizarFaseRevelado(puntuacion_reveal_phase_musa)
        : 0;
    const firma = `${estado.calculadoEnTs || 0}:${vista.paso}:${vista.tipo}:${fase}`;
    if (firma === puntuacion_firma_render_musa && opciones.forzar !== true) return;
    const pasoAnterior = Number(resultado_videojuego_musa_stage.dataset.step);
    const faseAnterior = Number(resultado_videojuego_musa_stage.dataset.phase);
    ...
    if (vista.tipo === "intro") {
        ...
        resultado_videojuego_musa_stage.innerHTML = `<div class="resultado-musa__intro"><span aria-hidden="true">&#x1F3AE;</span><small>RESULTADO DEL VIDEOJUEGO</small><h2>&iquest;QUI&Eacute;N JUG&Oacute; MEJOR?</h2><ul>${categorias}</ul>${marcadorTotalVideojuegoMusa(estado, 0)}</div>`;
    } else if (vista.tipo === "categoria") {
        const categoria = vista.categoria;
        const resultadoRevelado = fase >= 2;
        ganador = resultadoRevelado && !categoria.empate ? Number(categoria.ganador) : 0;
    ...
    if (cambioDeSlide) {
        requestAnimationFrame(() => {
            resultado_videojuego_musa_stage.classList.add("is-revealing");
            puntuacion_timeout_revelado_musa = setTimeout(() => {
                resultado_videojuego_musa_stage.classList.remove("is-revealing");
                puntuacion_timeout_revelado_musa = null;
            }, 1250);
        });
    }
    if (revelarEquipo) {
        const totalesObjetivo = api.totalesDuranteRevelado(estado, vista.indiceCategoria, fase);
...
function renderizarResultadoJuradoMusa(opciones = {}) {
    ...
    if (paso === 0) {
        ...
        resultado_jurado_musa_stage.innerHTML = `<div class="resultado-musa__apartado"><h2>${escapeHtml(criterio.label.toUpperCase())}</h2><div class="resultado-musa__cards">${tarjetaJuradoDirectoMusa(estado, 1, directo)}${tarjetaJuradoDirectoMusa(estado, 2, directo)}</div><p class="resultado-musa__veredicto">${!directo.confirmado ? "EL JURADO EST&Aacute; PUNTUANDO" : "PUNTUACIONES DEL APARTADO"}</p></div>`;
    } else {
        resultado_jurado_musa_stage.innerHTML = `<div class="resultado-musa__apartado resultado-musa__apartado--jurado"><small>VALORACI&Oacute;N ESC&Eacute;NICA</small><h2>PUNTUACIONES DEL JURADO</h2><div class="resultado-musa__cards">${tarjetaResumenJuradoMusa(estado.jugadores[1], 1)}${tarjetaResumenJuradoMusa(estado.jugadores[2], 2)}</div><p class="resultado-musa__veredicto">MEDIA DE LOS CRITERIOS ESC&Eacute;NICOS</p></div>`;
    }
    ...
    if (debeAnimar) requestAnimationFrame(() => {
        ...
        jurado_timeout_revelado_musa = setTimeout(() => {
            resultado_jurado_musa_stage.classList.remove("is-revealing");
            jurado_timeout_revelado_musa = null;
        }, 1250);
    });
    aplicarGanadorLocalMusa(resultado_jurado_musa, 0, firma);
...
function sincronizarVistaDeliberacionMusa(opciones = {}) {
    ...
    if (vista_modo_remota_musa === "deliberacion") activa = deliberacion_musa;
        ...
        renderizarResultadoJuradoMusa({ animar: true });
        activa = resultado_jurado_musa;
    } else if (vista_modo_remota_musa === "resultado_final") {
        renderizarResultadoFinalMusa({ animar: true });
...
function actualizarResultadoFinalMusa(payload = {}) {
    estado_resultado_final_musa = payload && typeof payload === "object" ? payload : {};
    if (vista_modo_remota_musa === "resultado_final") renderizarResultadoFinalMusa({ animar: true });
}
...
window.actualizarResultadoFinalMusa = actualizarResultadoFinalMusa;
...
function actualizarModoVistaMusaRemoto(payload = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, "puntuacion_slide_step")) {
        ...
        pasoCambiado = pasoCambiado || siguiente !== puntuacion_slide_step_musa;
        puntuacion_slide_step_musa = siguiente;
    }
    if (Object.prototype.hasOwnProperty.call(payload, "puntuacion_reveal_phase")) {
        const api = window.ScribFinalScore;
        const siguiente = api && typeof api.normalizarFaseRevelado === "function"
        ...
        pasoCambiado = pasoCambiado || siguiente !== puntuacion_reveal_phase_musa;
        puntuacion_reveal_phase_musa = siguiente;
    }
    if (Object.prototype.hasOwnProperty.call(payload, "jurado_slide_step")) {
        const siguiente = Math.max(0, Math.trunc(Number(payload.jurado_slide_step) || 0));
        pasoCambiado = pasoCambiado || siguiente !== jurado_slide_step_musa;
        jurado_slide_step_musa = siguiente;
    }
    ...
    if (modoAnterior !== vista_modo_remota_musa) {
        ...
        if (vista_modo_remota_musa === "puntuacion") renderizarPuntuacionFinalMusa({ animar: true });
        if (vista_modo_remota_musa === "resultado_jurado") renderizarResultadoJuradoMusa({ animar: true });
    }
    return true;
}
