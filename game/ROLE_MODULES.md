...
Sockets are created with `autoConnect: false` in `state.js` and connected at the end of `socket-events.js`. This guarantees all `socket.on(...)` handlers are registered before the `connect` event can fire.
